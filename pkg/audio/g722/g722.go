...
type Writer = audio.WriteCloser[Sample]
...
func Decode(w audio.PCM16Writer) (w2 Writer) {
	var f g722.Flags
	switch w.SampleRate() {
	case 8000:
		f |= g722.FlagSampleRate8000
	default:
		w = audio.ResampleWriter(w, 16000)
		fallthrough
	case 16000:
		// default
	}
	if dumpToFile {
		...
		w = audio.DumpWriterPCM16(pref+"_out", w)
		defer func() {
			w2 = audio.DumpWriter[Sample]("g722", pref+"_in", w2)
		}()
	}
...
func (e *Encoder) WriteSample(in audio.PCM16Sample) error {
...
func Encode(w Writer) (w2 audio.PCM16Writer) {
	var f g722.Flags
	switch w.SampleRate() {
	default:
	...
	if dumpToFile {
		...
		w = audio.DumpWriter[Sample]("g722", pref+"_out", w)
		defer func() {
			w2 = audio.DumpWriterPCM16(pref+"_in", w2)
		}()
	}
