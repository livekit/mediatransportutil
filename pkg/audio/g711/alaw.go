// Copyright 2024 LiveKit, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// 	http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package g711

import (
	"fmt"
	"io"

	prtp "github.com/pion/rtp"

	"github.com/livekit/mediatransportutil/pkg/audio"
	"github.com/livekit/mediatransportutil/pkg/audio/rtp"
)

const ALawSDPName = "PCMA/8000"

func init() {
	audio.RegisterCodec(rtp.NewAudioCodec(audio.CodecInfo{
		SDPName:     ALawSDPName,
		SampleRate:  8000,
		RTPDefType:  prtp.PayloadTypePCMA,
		RTPIsStatic: true,
		Priority:    -20,
		FileExt:     "g711a",
	}, DecodeALaw, EncodeALaw))
}

type ALawSample []byte

func (s ALawSample) Size() int {
	return len(s)
}

func (s ALawSample) CopyTo(dst []byte) (int, error) {
	if len(dst) < len(s) {
		return 0, io.ErrShortBuffer
	}
	n := copy(dst, s)
	return n, nil
}

func (s ALawSample) Decode() audio.PCM16Sample {
	out := make(audio.PCM16Sample, len(s))
	DecodeALawTo(out, s)
	return out
}

func (s *ALawSample) Encode(data audio.PCM16Sample) {
	out := make(ALawSample, len(data))
	EncodeALawTo(out, data)
	*s = out
}

type ALawWriter = audio.WriteCloser[ALawSample]

type ALawDecoder struct {
	w   audio.PCM16Writer
	buf audio.PCM16Sample
}

func (d *ALawDecoder) String() string {
	return fmt.Sprintf("PCMA(decode) -> %s", d.w)
}

func (d *ALawDecoder) SampleRate() int {
	return d.w.SampleRate()
}

func (d *ALawDecoder) Close() error {
	return d.w.Close()
}

func (d *ALawDecoder) WriteSample(in ALawSample) error {
	if len(in) >= cap(d.buf) {
		d.buf = make(audio.PCM16Sample, len(in))
	} else {
		d.buf = d.buf[:len(in)]
	}
	DecodeALawTo(d.buf, in)
	return d.w.WriteSample(d.buf)
}

func DecodeALaw(w audio.PCM16Writer) ALawWriter {
	switch w.SampleRate() {
	default:
		w = audio.ResampleWriter(w, 8000)
	case 8000:
	}
	return &ALawDecoder{w: w}
}

type ALawEncoder struct {
	w   ALawWriter
	buf ALawSample
}

func (e *ALawEncoder) String() string {
	return fmt.Sprintf("PCMA(encode) -> %s", e.w)
}

func (e *ALawEncoder) SampleRate() int {
	return e.w.SampleRate()
}

func (e *ALawEncoder) Close() error {
	return e.w.Close()
}

func (e *ALawEncoder) WriteSample(in audio.PCM16Sample) error {
	if len(in) >= cap(e.buf) {
		e.buf = make(ALawSample, len(in))
	} else {
		e.buf = e.buf[:len(in)]
	}
	EncodeALawTo(e.buf, in)
	return e.w.WriteSample(e.buf)
}

func EncodeALaw(w ALawWriter) audio.PCM16Writer {
	switch w.SampleRate() {
	default:
		panic("unsupported sample rate")
	case 8000:
	}
	return &ALawEncoder{w: w}
}
