...
func Generate(buf audio.PCM16Sample, ts, dur time.Duration, amp int16, freq []Hz) time.Duration {
	for i := range buf {
...
func Play(ctx context.Context, writer audio.Writer[audio.PCM16Sample], vol int16, tones []Tone) error {
	const (
		frameDur     = rtp.DefFrameDur
		framesPerSec = int(time.Second / frameDur)
	)
	pcmBuf := make(audio.PCM16Sample, writer.SampleRate()/framesPerSec)
	...
	for {
		...
		if err := writer.WriteSample(pcmBuf); err != nil {
			return err
		}
		remaining -= frameDur
		ts += frameDur
