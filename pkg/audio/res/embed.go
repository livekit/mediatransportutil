...
func ReadOggAudioFile(data []byte) []audio.PCM16Sample {
	const perFrame = SampleRate / audio.DefFramesPerSec
	...
	var samples audio.PCM16Sample
	buf := make([]float32, perFrame)
	for {
		n, err := r.Read(buf)
		if n != 0 {
			frame := make(audio.PCM16Sample, n)
			for i := range frame {
