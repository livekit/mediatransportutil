...
func (p *paceWriter) WriteSample(sample audio.PCM16Sample) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func TestResample(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(strconv.Itoa(c.Rate), func(t *testing.T) {
			...
			var dstFrames []audio.PCM16Sample
			dst := audio.NewPCM16FrameWriter(&dstFrames, c.Rate)
			pw := &paceWriter{w: dst}
			r := audio.ResampleWriter(pw, srcRate)
			defer r.Close()
			totalSamples := 0
...
func writePCM16s(t testing.TB, path string, buf []audio.PCM16Sample) string {
	var out []byte
...
func writePCM16sWebm(t testing.TB, path string, rate int, buf []audio.PCM16Sample) {
	f, err := os.Create(path)
	require.NoError(t, err)
	defer f.Close()

	w := webm.NewPCM16Writer(f, rate, audio.DefFrameDur)
	defer w.Close()
...
func TestResampleLeak(t *testing.T) {
	...
	for run := range runs {
		func() {
			...
			var dstFrames []audio.PCM16Sample
			dst := audio.NewPCM16FrameWriter(&dstFrames, 16000)
			r := audio.ResampleWriter(dst, srcRate)
