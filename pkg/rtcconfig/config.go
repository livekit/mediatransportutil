...
type RTCConfig struct {
	...
	AdditionalHostIPs        []string         `yaml:"additional_host_ips,omitempty"`
...
func (conf *RTCConfig) Validate(development bool) error {
	if err := conf.validateAdditionalHostIPs(); err != nil {
		return err
	}
...
func (conf *RTCConfig) validateAdditionalHostIPs() error {
	if len(conf.AdditionalHostIPs) == 0 {
		return nil
	}

	ips := make([]string, 0, len(conf.AdditionalHostIPs))
	seen := make(map[string]struct{}, len(conf.AdditionalHostIPs))
	for _, raw := range conf.AdditionalHostIPs {
		ip := net.ParseIP(strings.TrimSpace(raw))
		if ip == nil {
			return fmt.Errorf("invalid additional host ip %q", raw)
		}
