...
type RTCConfig struct {
	...
	// SkipExternalIPValidation disables the self-hairpin probe that runs
	// after STUN binding succeeds (validateExternalIP). When the deployment
	// topology cannot self-loopback to its own external IP — for example
	// hostNetwork pods behind an edge firewall without NAT reflection — the
	// validation step times out even though the discovered IP is correct.
	// Setting this to true trusts the STUN-reported address and skips the
	// validation. Operators opting in are accepting the misconfiguration
	// surface that comes with not verifying reachability.
