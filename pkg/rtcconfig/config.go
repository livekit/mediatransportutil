...
type RTCConfig struct {
	...
	UseNodeIPAsExternalIP   bool             `yaml:"use_node_ip_as_external_ip"`
	NAT1to1IPs              []string         `yaml:"nat_ip_mapping,omitempty"`
...
func (conf *RTCConfig) Validate(development bool) error {
	...
	for i, ipMapping := range conf.NAT1to1IPs {
		spl := strings.Split(ipMapping, "/")
		if len(spl) != 2 {
			return fmt.Errorf("unable to parse mapping '%s' at index %d", ipMapping, i)
		}
		left := spl[0]
		if net.ParseIP(left) == nil {
			return fmt.Errorf("unable to parse left IP '%s' at index %d", left, i)
		}

		right := spl[1]
		if net.ParseIP(right) == nil {
			return fmt.Errorf("unable to parse right IP '%s' at index %d", right, i)
		}
	}
